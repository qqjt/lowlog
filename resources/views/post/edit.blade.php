    ...
    <div class="container">
        <form id="edit-post-form" method="post" action="{{route('post.update', [$post])}}">
            ...
            <div class="form-group{{ $errors->has('excerpt') ? ' has-error' : '' }}">
                <label for="content">{{__('Excerpt')}}</label>
                <textarea name="excerpt" id="excerpt" class="form-control" rows="3"
                          placeholder="{{__('Excerpt')}}">{{$post->excerpt}}</textarea>
                    ...
                    @foreach($categories as $category)
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" value="{{$category->id}}"
                                   id="category-{{$category->id}}" name="categories[]" @if($post->categories->contains('id', $category->id)) checked @endif>
